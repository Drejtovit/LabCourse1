...
export async function DELETE(request) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ success: false, errors: { id: "Experience ID is required" } }, { status: 400 });
    }

    if (!(await prisma.experience.findUnique({ where: { id: parseInt(id) } }))) {
      return NextResponse.json({ success: false, errors: { id: "Experience not found" } }, { status: 404 });
    }
